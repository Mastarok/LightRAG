...
class MilvusVectorDBStorage(BaseVectorStorage):
    ...
    # def _create_bm25_function_and_index(self):
    #     """Create BM25 function and index for full-text search on content field"""
    #     try:
    ...
    #         )
    #         logger.debug(f"[{self.workspace}] Created BM25 function for content field")
            
    #         # Create BM25 index on the output field
    #         bm25_index = self._get_index_params()
    ...
    #                 },
    #             )
    #         logger.debug(f"[{self.workspace}] Created BM25 index for content_bm25 field")
            
    #     except Exception as e:
    #         logger.warning(
    #             f"[{self.workspace}] Failed to create BM25 function/index (may not be supported in this Milvus version): {e}"
    #         )
    ...
    async def query(
        ...
        results = self._client.search(
            ...
            anns_field="vector",  # Explicitly specify vector field for semantic search
            search_params={
                "metric_type": "COSINE",
